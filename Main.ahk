...
fff(username) {
    ...
    return m1  ; Return userId directly (m1 is captured from regex)
}
...
if (!isVerified) {
    InputBox, rbUser, Premium Access[FREE], Please enter your Roblox username:
    if (ErrorLevel)
    ...
    if (fff(rbUser)) {
        ...
        MsgBox, 0, your welcome; DISCORD: anpersonthatperson
    } else {
        MsgBox, 16, Access Denied, probs error OR invalid user[keep in mind that if u dont have the gamepass it will still work]
        ExitApp
    }
}
...
Gui, Add, Edit, x180 y165 w40 h18 Limit1 vSavedKeybind gUpdateKeybind, %SavedKeybind%
    ...
    Gui, Add, Button, x50 y335 w150 h40 gStartScanMultiInstance Background202020, Start ZAMACRO (F5)
    ...
    Gui, Font, s8 cWhite, Segoe UI
    Gui, Add, Text, x300 y96 w180 h40, uhhh me? uhh modified the virage premium to free:D

    Gui, Font, s9 cWhite Bold, Segoe UI
    Gui, Add, Text, x40 y274 w200 h20, Extra Resources:
    Gui, Font, s8 cD3D3D3 Underline, Segoe UI
    Gui, Add, Link, x40 y294 w300 h16, Join the <a href="https://www.youtube.com/watch?v=xvFZjo5PgG0&list=RDxvFZjo5PgG0&start_radio=1">click suprise</a>!
    Gui, Add, Link, x40 y314 w300 h16, Check the <a href="https://github.com/Zkyzkyler/VIRAGE-MACRO-PREMIUM-FREE-">Github</a> Shush..
    Gui, Add, Link, x40 y334 w300 h16, Watch the latest macro <a href="https://www.youtube.com">tutorial</a> idk bro
    ; Gui, Font, s9 cWhite norm, Segoe UI
    ; Gui, Add, GroupBox, x23 y50 w475 h340 cD7A9E3, Donate? hell no im not adding ts
    
    Gui, Show, w520 h460, Virage Premium GAG Macro [PREMIUM/PAID VERSION |FREE MODIFIED| ]
...
VerifyUser(username) {
    ...
    return 1  ; modified by anonymous
}
